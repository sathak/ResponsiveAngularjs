    ...
    $.fn.columnFilter = function (options) {
        ...
        function _fnGetColumnValues(oSettings, iColumn, bUnique, bFiltered, bIgnoreEmpty) {
            ...
            for (var i = 0, c = aiRows.length; i < c; i++) {
                ...
                if(sValue === undefined)
                    sValue = Object.keys(aData)[iColumn];
        ...
        function fnCreateDateRangeInput(oTable) {
            ...
            $('#' + sFromId, th).change(function () {
               ...
               dropDownId = this.id;
               dropdownIndex = dropDownId.split("_").pop();
                ...
                oTable.fnDraw();
                fnOnFiltered();
                //oTable.fnFilter(this.value, _fnColumnIndex(i),false , false); //Issue 37
                //fnOnFiltered();
            });
	        ...
	        function (oSettings, aData, iDataIndex) {
	            ...
	            var dStartDate = $("#"+dropDownId).datepicker("getDate");
                ...
                var dCellDate = null;
	            try {
	                if (aData[_fnColumnIndex(dropdownIndex)] == null || aData[_fnColumnIndex(dropdownIndex)] == "")
	                    return false;
	                dCellDate = $.datepicker.parseDate($.datepicker.regional[""].dateFormat, aData[_fnColumnIndex(dropdownIndex)]);
	            } catch (ex) {
	                return false;
	            }
	            if (dCellDate === null || dStartDate === null)
	                return false;
                
                
                if (dCellDate.getDate() === dStartDate.getDate()) {
	                return true;
	            }
	            ...
	            var dEndDate = $("#"+dropDownId).datepicker("getDate");
...
*/
        ...
        function fnCreateColumnSelect(oTable, aData, iColumn, nTh, sLabel, bRegex, oSelected, bMultiselect) {
            var iTable = oTable.DataTable();
                
            //iTable.column(3).data().unique().sort()
            
            if (aData == null){
                aData = iTable.column(iColumn).data().unique().sort();
        ...
        return this.each(function () {
            ...
            if (properties.sPlaceHolder == "head:after") {
                mainTr = $("tr:first", oTable.fnSettings().nTHead).html();
                ...
                if (oTable.fnSettings().bSortCellsTop) {
                    ...
                    tr.appendTo($(oTable.fnSettings().nTHead));
                    tr1.appendTo($(oTable.fnSettings().nTHead));
                    
                          //tr.clone(true).appendTo(oTable.fnSettings().nTHead)
                    //tr.clone(true).appendTo(oTable.fnSettings().nTHead)
                    //tr.prependTo($("thead", oTable));
                    aoFilterCells = oTable.fnSettings().aoHeader[0];
                    //headerCells = $(oTable.fnSettings().aoHeader[0]).html();
                }
            ...
            $("tr:last th", oHost).each(function (index) {//bug with ColVis
            //$(aoFilterCells).each(function (index) {//fix for ColVis
                ...
                if ((aoColumn === null || aoColumn.sSelector == null)) {
                ...
                if(label.toLocaleLowerCase() === "edit" || label.toLocaleLowerCase() === "delete"){
                   // $(th[0]).html("");
                    $(th[0]).css("vertical-align", "middle");
                    $(th[0]).css("text-align", "start");
                    $(th[0]).attr("rowspan", "2");
                    return true;
           ...
           /* tr.clone(true).appendTo(oTable.fnSettings().nTHead);
            $("span",$("tr:first", oTable.fnSettings().nTHead)).remove();*/
            
            
            $("tr:first", oTable.fnSettings().nTHead).children().each(function (index){
                //$(this).text($($(mainTr)[index]).text())
                var label = $(this).text();
                if(label.toLocaleLowerCase() === "edit" || label.toLocaleLowerCase() === "delete"){
                   // $(th[0]).html("");
            ...
            var hTr =  $("tr:last", oTable.fnSettings().nTHead);
            
            $("tr:last", oTable.fnSettings().nTHead).children().each(function (index){
               if($(this).html() === ""){
                   $(this).remove();
               }
               var disp =  $($("tr:first", oTable.fnSettings().nTHead).children()[index]).css("display");    
               if(disp === undefined || disp === "none")
                   $(this).remove();
            });
            
            //$("tr:last", oTable.fnSettings().nTHead).find("span").remove();
            
            hTr.find("input").css("height","30px");
            hTr.find("select").css("height","30px");
            hTr.find("th").css("padding-top","4px");
            hTr.find("th").css("padding-bottom","2px");
            hTr.find("th").css("background-color","red");
            hTr.find("th").css("background","none");
            hTr.find("th").css("padding-right","2px");
            hTr.find("th").css("padding-left","1px");

        });

    };

    $('body').on('focus',".date_range_filter", function(){
        $(this).datepicker();
    });
