(function (navcon) {
    navcon.kendo.tree = {
        ...
        columnSetting: function (settings, scope) {
            ...
            $.each(settings.columns, function (i, item) {
                ...
                if (item.template !== undefined && item.template.toLowerCase() === "checkbox") {
                    ...
                    item.headerTemplate = "<span class='k-link'><input type='checkbox' ng-text='" + item.field + "' onclick='navcon.kendo.tree.toggleAll(event)' />&nbsp;&nbsp;" + item.title + "</span>";
                    //item.template = $("#treeview-checkbox-template").html();
                    //#= Discontinued ? "checked=checked" : "" # disabled="disabled"
                    item.template = "<input type='checkbox' class='treeview-checkbox' ng-model='" + item.field + "' data-bind='checked: " + item.field + "' onclick='navcon.kendo.tree.toggleItem(event)' />";
                }
        ...
        rowSetting: function (settings, scope) {
            var items = [];
            var treeData = angular.copy(settings.data);
            items = navcon.getDataFromTree(treeData, '', 0);

            $.each(items, function (i, item) {
                item.id = item.data.id;
                item.Add = item.data.Add;//(item.data.Add === "1" ? true : false);
                ...
                if (settings.editable !== undefined && settings.editable.enabled !== undefined && settings.editable.enabled) {
                    ...
                    oTree.setOptions({
                        ...
                        edit: function (e) {
                            ...
                            var approvalColumn = navcon.kendo.tree.approvalColumn(scope.tableSettings.columns);
                ...
                for (var iCol = 0; iCol < columns.length; iCol++) {
                    if (columns[iCol] !== undefined && columns[iCol].checkDeny !== undefined && columns[iCol].checkDeny !== "") {
                        if (columns[iCol].checkDeny) {
                            view[selectedRowIndex].set(columns[iCol].field, false);
                        } else if (!columns[iCol].checkDeny) {
                            view[selectedRowIndex].set(columns[iCol].field, true);
                        }
                    }
                }
                //view[selectedRowIndex].set(columns[selectedColumnIndex].field, true);
            }
        },
